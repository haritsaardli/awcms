...
export interface ExtensionData {
    ...
    description?: string;
    version: string;
    category: 'ecommerce' | 'crm' | 'marketing' | 'analytics' | 'social' | 'utility' | 'custom';
...
export async function getExtensions(
    ...
    return (data || []).map((ext) => ({
        ...
        config: typeof ext.config === 'string' ? JSON.parse(ext.config) : (ext.config || {}),
 ...
 */
export function createExtensionRegistry(extensions: ExtensionData[]): ExtensionRegistry {
